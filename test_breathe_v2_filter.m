...
[b_low,a_low] = butter(9,0.67/Fs);
[z,p,k] = butter(9,0.33/Fs,'high');

% freqz(b_high,a_high)

% figure(1)
% highpass(CSI_sanit_pha,0.33,Fs);
% CSI_sanit_pha = highpass(CSI_sanit_pha,0.033,Fs);
CSI_sanit_pha = filter(z,p,k,CSI_sanit_pha);

% figure(2)
% CSI_sanit_pha = lowpass(CSI_sanit_pha,0.067,Fs);
CSI_sanit_pha = filter(b_low,a_low,CSI_sanit_pha);
